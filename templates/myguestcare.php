...
<div class="astro_be <?php echo $astro_be_prefix . $provider; ?>">
    ...
    <form class="astro_be_form astro_be_form<?php echo '_'. esc_attr($provider); ?>"
          ...
          action="https://booking.myguestcare.com/.eshop"
          target="<?php echo esc_attr( get_option($astro_be_prefix.$provider.'_form_target') ); ?>">

        <input type="hidden" name="idcliente" value="<?php echo esc_attr( get_option($astro_be_prefix.$provider.'_idcliente') ); ?>" />
        <input type="hidden" name="act" value="result-strutture" />
        <input type="hidden" name="orderBy" value="2" />
        <input type="hidden" name="fast" value="1" />
        <input type="hidden" name="invcart" value="1" />
        <input type="hidden" name="idvendor" value="1" />
        <input type="hidden" name="alltratt" value="1" />
        <input type="hidden" name="noFilterUnit" value="1" />
        <input type="hidden" name="richinfopren" value="on" />
        <input type="hidden" name="stanze" value="1" />

        <input type="hidden" id="astro_be_form<?php echo '_'. esc_attr($provider); ?>_arrivo" name="arrivo" value="" />
        <input type="hidden" id="astro_be_form<?php echo '_'. esc_attr($provider); ?>_partenza" name="partenza" value="" />
        ...
        <div class="<?php echo $astro_be_prefix . 'row'; ?> <?php echo $astro_be_prefix . 'occupancy'; ?>">
            ...
            $field_name = 'adulti1'; //provider field name
            if ($adults_enable) {
                ?>
                <!-- <?php echo $astro_be_prefix.$field_class; ?> -->
                <div class="<?php echo $astro_be_prefix . 'column ' . $astro_be_prefix . 'column-' . $field_class; ?>">
            ...
            $field_name = 'bambini1'; //provider field name
            if ($children_enable) {
                ?>
                <!-- <?php echo $astro_be_prefix.$field_class; ?> -->
                <div class="<?php echo $astro_be_prefix . 'column ' . $astro_be_prefix . 'column-' . $field_class; ?>">
        ...
        $field_name = 'etabambino1'; //provider field name: etabambino + room number (1) + child number
                ...
                for ($x = 1; $x <= $children_n_max; $x++) {
                    ...
                    <div class="<?php echo $astro_be_prefix . 'column ' . $astro_be_prefix . 'column-' . $field_class; ?> <?php echo $astro_be_prefix . 'column-' . $field_class . '-' . esc_attr($x); ?>">
                        <div class="<?php echo $astro_be_prefix . 'column-inner'; ?>">
                            ...
                            <select name="<?php echo esc_attr($field_name . $x); ?>" id="<?php echo $astro_be_prefix . $provider . '-' . $field_class . '-' . esc_attr($x); ?>" class="<?php echo $astro_be_prefix . 'select'; ?> <?php echo $astro_be_prefix . 'select-' . $field_class; ?>" size="1">
